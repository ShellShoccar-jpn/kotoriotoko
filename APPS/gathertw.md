...
```
-c,--continuously:
    検索条件で指定された最古のツイートまで検索し終わると、現時点のツイートから
    現時点の最新ツイートの範囲で再建策する。
    このオプションを指定すれば、擬似的なリアルタイム検索ができる。
    <過去ツイートの検索が要らない場合>
    一度本コマンドを実行して1個以上ツイートが獲得できたら[CTRL]+[C]を押して
...
--sinceid=<ツイートID>:
    ...
    また、-cオプション指定時は、条件に基づく過去の検索が終わると、
    2週目以降の再実行時には、データ格納ディレクトリー内にある最新ツイートのIDが
...
まず、検索できなくなってしまう前にゴールデンウィーク序盤（2016/04/30まで）のツイートを先に収集してしまい、それが終わったらそれ以降のツイートをエンドレスで集める（収集先ディレクトリーは `GW2016` ）。ただし、“仕事”を含むツイートは除外したい。
